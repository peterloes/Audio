 ...
 * - Up to @ref MAX_SEC_TIMERS software timers with callback functionality
 *   and a granularity of one second.
 * - One high-resolution timer for short time measurements, e.g. timeout or
 *   autorepeat features for keys (push buttons).
 * - Up to @ref MAX_ALARMS alarm times with callback functionality and a
 *   granularity of one minute (repeated after 24h).
 *
 * @note
 * The index for specifying a dedicated alarm time (i.e. the <b>alarmNum</b>
 * parameter) is an integer value between 0 and (@ref MAX_ALARMS - 1).  It is
...
int	i;	// index variable
    ...
    if (l_MaxHdl >= (MAX_SEC_TIMERS - 1))
	LogError("sTimerCreate(): No more Timer Handles (%d)", MAX_SEC_TIMERS);
#endif
...
#ifdef LOGGING
    /* Check if such a function has been defined already */
    if (l_msTimerFunction != NULL  &&  l_msTimerFunction != function)
	Log ("WARNING: msTimerAction(): ANOTHER FUNCTION IS ALREADY ASSIGNED");
#endif
...
/***************************************************************************//**
 *
 * @brief	Get start value for a milliseconds delay
 *
 * This routine returns the current RTC value as start count for
 * msDelayIsDone().  It is used for non-blocking delays.
 *
 * @return
 *	Actual value of the Real-Time-Counter.
 *
 ******************************************************************************/
uint32_t msDelayStart (void)
{
    /* Return current time counter values */
    return RTC->CNT;
}

/***************************************************************************//**
 *
 * @brief	Check if specified amount of milliseconds is over
 *
 * This routine checks if the specified amount of milliseconds has elapsed.
 * It is used for non-blocking delays.  First call msDelayStart() to get the
 * start count for the delay calculation.
 *
 * @param[in] startCnt
 *	Start count for the delay calculation.  Use msDelayStart() to get
 *	this value.
 *
 * @param[in] ms
 *	Duration in milliseconds to specify delay.
 *
 * @return
 * 	Comparison: <i>true</i> if specified delay is done, <i>false</i> if not.
 *
 ******************************************************************************/
bool	msDelayIsDone (uint32_t startCnt, uint32_t ms)
{
uint32_t msTics;

    /* Parameter check */
    if (ms == 0)
	return true;		// immediate return if no delay

    EFM_ASSERT (0 < ms  &&  ms <= MAX_VALUE_FOR_32BIT);

    /* Convert the [ms] value in number of ticks */
    msTics = ((ms * RTC_COUNTS_PER_SEC) / 1000) & 0xFFFFFF;

    /* Check if delay is done */
    return ((RTC->CNT - startCnt) & 0xFFFFFF) < msTics ? false : true;
}
...
uint32_t startCnt, msTics;
    ...
    while (((RTC->CNT - startCnt) & 0xFFFFFF) < msTics)
	;
}
 ...
 *
 * @param[in] pMsVar
 *	Pointer to a variable where to store the milliseconds part.
 *
 ******************************************************************************/
